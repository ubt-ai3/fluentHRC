...
public:
    ...
    [[nodiscard]] unsigned int getRowsCount() const { return rows_count; }
    // This gets count of columns.
    [[nodiscard]] unsigned int getColumnsCount() const { return cols_count; }
	...
	// This gets pointer to row's field ( typedef std::vector<std::string> row; )
	// with row index row_n and field(column) index col_n. Null if field does not exist
	const std::string* getFieldOrNull(unsigned int row_n, unsigned int col_n);
