...
namespace pcl
{
    ...
    class Kinect2Grabber : public pcl::Grabber
    {
        public:
			...
			typedef void ( signal_Kinect2_MatRGB )(const std::shared_ptr<cv::Mat4b>&);
        ...
        protected:
			...
			std::shared_ptr<cv::Mat4b> convertRGBToMatRGB(INT64 timestamp, RGBQUAD* colorBuffer, bool mirror);
    ...
    void pcl::Kinect2Grabber::threadFunction()
    {
        while( !quit ){
            ...
            if( SUCCEEDED( result ) ){
				...
				result &= colorFrame->get_RelativeTime(&timestamp);
	...
	std::shared_ptr<cv::Mat4b> pcl::Kinect2Grabber::convertRGBToMatRGB(INT64 timestamp, RGBQUAD* colorBuffer, bool mirror)
	{
		std::shared_ptr<cv::Mat4b> mat(new cv::Mat4b(colorHeight, colorWidth));
